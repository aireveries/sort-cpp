...
void OnlineSortTracker::process(const std::vector<TrackingBox>& frame) {
    total_frames++;
    frame_count++;

    // We see the first frame
    if (trackers.size() == 0) {
    ...
    for (auto tb : frameTrackingResult)
        results << tb.frame << "," << tb.id << "," << tb.box.x << "," << tb.box.y << "," << tb.box.width << "," << tb.box.height << ",1,-1,-1,-1" << endl;

}

void OnlineSortTracker::processFirstFrame(const std::vector<TrackingBox>& frame) {
    ...
    for (unsigned int id = 0; id < frame.size(); id++) {
        TrackingBox tb = frame[id];
        results << tb.frame << "," << id + 1 << "," << tb.box.x << "," << tb.box.y << "," << tb.box.width << "," << tb.box.height << ",1,-1,-1,-1" << endl;
    }
}

void OnlineSortTracker::print(string& result) {
    result = results.str();
}